...
export class Study {
    constructor(words) {
        ...
        this.types = {
            ...
            HeqASS(word) {
                if (word.levelProgress === 3) {
                    word.levelProgress--;
                    return 'error in HeqASS'
                }
                console.log('hearing => association ' + word.word);
                word.levelProgress++;
            },
    ...
    newWordsLearning(words, index = 0, left = 15, number = 5) {
        ...
        const newWords = words || this.shuffleArray(this.words.filter(word => word.level === 0).slice(0, number).flatMap(word => Array(wordsLength).fill(word)));
    ...
    halfWordsLearning(words, index = 0, left = 15, number = 5) {
        ...
        const halfWords = words || this.shuffleArray(this.words.filter(word => 0 < word.level < 4).slice(0, number));
        ...
        this.halfWordsLearning(halfWords.filter(word => word.level < 4), index+1, left-1);
